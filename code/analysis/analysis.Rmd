...
- analysis
  - demographics
  - ensure BCA is used throughout
  - add pairwise comparisons using emmeans - see old code in IRAP repo
...
p_cis_by_domain <- 
  ...
  arrange(estimate) %>%
  group_by(domain, measure, method) %>%
...
data_diff_zero <- 
  ...
  summarize(proportion_diff_zero = mean(sig, na.rm = TRUE),
            ...
            .groups = "drop") %>%
  mutate(variance = ifelse(variance == 0, 0.000001, variance))  # models cannot be run on zero variance, so offset by a minuscule amount
...
p_prop_nonzero <- 
  ...
  scale_shape_manual(values = c(17, 16, 15, 19)) +
  scale_color_viridis_d(alpha = 1, begin = 0.2, end = 0.8, direction = -1) +
  labs(x = "",
...
data_processed %>%
  group_by(measure) %>%
  summarize(min_observed_score = min(estimate),
            max_observed_score = max(estimate)) %>%
  mutate(observed_range_score = max_observed_score - min_observed_score) %>% 
  round_df(2) %>%
  #gather() %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
...
fit_ci_width_proportions <- 
  lmer(ci_width_proportion ~ 1 + measure + (1 | domain), 
...
p_ci_width_proportion_observed_range <- 
  results_ci_width_proportions %>%
  ggplot(aes(fct_rev(measure), estimate)) +
  #geom_linerange(aes(ymin = pi_lower, ymax = pi_upper), size = 0.5, linetype = "solid") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper)) +
  geom_point(size = 2.5) +
  mdthemes::md_theme_linedraw() +
