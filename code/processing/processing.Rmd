...
# 
# write_rds(raw_trial_level_df, "../../data/impraw.rds", compress = "gz")
# write_rds(raw_sessions_df, "../../data/allds.rds")

raw_trial_level_df <- read_rds("../../data/impraw.rds")
raw_sessions_df <- read_rds("../../data/allds.rds")
...
# compute scores from zero 

```{r}

n_boots = 100 # for testing. change to 2000
subset_sample = TRUE # to reduce processing time for testing

if(!file.exists("../../data/processed/data_processed.rds")){
  ...
  bootstrap_A_score <- function(data){
    ...
    output <- 
      tibble(method   = c("bca"),
             estimate = fit$t0,
             ci_lower = c(results$bca[2]),
             ci_upper = c(results$bca[3]))
  ...
  # save to disk
  write_rds(A_scores, "../../data/processed/data_processed.rds")
