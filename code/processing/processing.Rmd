...
split_half_scores <- critical_trials_complete_tasks |>
  group_by(SESSION_ID, domain, measure) |>
  mutate(trial_n = row_number()) |>
  ungroup() |>
  mutate(half = case_when(measure == "amp"  & trial_n <= 24 ~ "first",
                          measure == "iat"  & trial_n <= 60 ~ "first",
                          measure == "biat" & trial_n <= 64 ~ "first",
                          measure == "siat" & trial_n <= 96 ~ "first",
                          measure == "gnat" & trial_n <= 80 ~ "first",
                          measure == "ept"  & trial_n <= 90 ~ "first",
                          TRUE ~ "second")) |>
  mutate(session_id = as.factor(SESSION_ID),
         trial_consistency = as.character(trial_consistency)) |>
  ...
  summarize(score = A_score(.)) |>
  ungroup()

split_half_scores_reshaped <- split_half_scores |>
